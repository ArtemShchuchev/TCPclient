...
QDataStream &operator >> (QDataStream &out, StatServer &s)
{
    /*
    ...
    */
    out >> s.incBytes >> s.sendBytes >> s.revPck >> s.sendPck >> s.workTime >> s.clients;
    return out;
};
...
{
    QByteArray sendHdr;
    QDataStream outData(&sendHdr, QIODevice::WriteOnly);
